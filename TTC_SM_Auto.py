...
class TTC_SM_Auto:
    def main(self,num_samples, set_voltage, Father):
        ...
        test_mode = "Auto"
        head = []
        head.append(["Time(s)", "SetTemp.(oC)", "Actual Temp.(oC)", "Humidity(%)", "Temp.Sensor(oC)", "Sample number", "Current(A)", "Resistence(ohm)"])
        ...
        for step in TP:
            ...
            while( t1 < t2 ):
                ...
                for i in range(self.NEXTION_NUM_SAMPLES):
                    ...
                    R_Value = format(self.NEXTION_SET_VOLTAGE / float(I_Value), ".4E")
